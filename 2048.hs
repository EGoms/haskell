...
shift l = take (length l) $ combine (filter (>0) 1) ++ [0, 0..]
...
loss, win :: Position -> Bool
loss pos = all isNothing [progress move pos| move <-[left, right, up, down] ]
...
indicesOf2 :: [[a]] -> [ReifiedTraversal' [[a]] a]
indicesOf2 ls = [ Traversal $ i.j | Traversal i <- indicesOf ls, let Just l = ls ^? i, Traveral j <- indicesOf l]

add2or4 :: Position -> RVar Position
add2or4 pos = do
  xy <- randomElement [xy | Traversal xy <- indicesOf2 pos, pos ^? xy == Just 0 ]
  a <- categorial [(1-prob4, 2), (prob4, 4) ]
  return $ pos & xy .~a
